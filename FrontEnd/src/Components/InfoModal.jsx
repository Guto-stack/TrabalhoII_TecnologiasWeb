import {X, MapPin, Scale, Ruler, Calendar, Utensils, Search } from 'lucide-react';
                ...
                <div className="p-6 overflow-y-auto custom-scrollbar flex flex-col gap-6 text-text-main">
                    {isEspecie && (
                        <div className="flex flex-col md:flex-row gap-6">
                            ...
                            <div className="flex-1 grid grid-cols-2 gap-3">
                                ...
                                {/* Card Dieta */}
                                <div className="flex flex-col gap-1.5 bg-canvas p-3 rounded-xl border border-accent/10 shadow-sm">
                                    <div className="flex items-center gap-1.5 opacity-70">
                                        <Utensils className="w-4 h-4 text-accent shrink-0" />
                                        <span className="text-xs font-bold uppercase tracking-wider">Dieta</span>
                                    </div>
                                    <span className="text-sm font-semibold leading-tight">{data.dieta || 'Desconhecida'}</span>
                                </div>

                                {/* Card Dimensões (Comprimento e Altura) */}
                                <div className="flex flex-col gap-1.5 bg-canvas p-3 rounded-xl border border-accent/10 shadow-sm">
                                    <div className="flex items-center gap-1.5 opacity-70">
                                        <Ruler className="w-4 h-4 text-accent shrink-0" />
                                        <span className="text-xs font-bold uppercase tracking-wider">Dimensões</span>
                                    </div>
                                    <div className="flex flex-col text-sm font-semibold leading-tight gap-0.5">
                                        <span>Comp: {data.comprimento ? `${data.comprimento} m` : '---'}</span>
                                        <span>Alt: {data.altura ? `${data.altura} m` : '---'}</span>
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-col gap-1.5 bg-canvas p-3 rounded-xl border border-accent/10 shadow-sm">
                                    ...
                                    <span className="text-sm font-semibold leading-tight">{data.peso ? `${data.peso} kg` : '---'}</span>
                                </div>
                                ...
                                <div className="flex flex-col gap-1.5 bg-canvas p-3 rounded-xl border border-accent/10 shadow-sm">
                                    <div className="flex items-center gap-1.5 opacity-70">
                                        ...
                                        <span className="text-xs font-bold uppercase tracking-wider">Descoberta</span>
                                    </div>
                                    <span className="text-sm font-semibold leading-tight">{data.descoberta || '---'}</span>
                                </div>
                    ...
                    {isEspecie && data.localidade_exata && (
                        <div>
                            ...
                            <div className="flex items-center gap-2 bg-canvas p-4 rounded-xl border border-accent/10">
                                ...
                                {!isMapaRoute && (
                                    <button 
                                        onClick={() => {
                                            onClose();
                                            navigate('/mapa', {state: {searchEspecie: data.nomeCientifico}});
                                        }} 
                                        className="ml-auto shrink-0 px-4 py-2 text-xs font-bold bg-accent text-node rounded-lg hover:brightness-110 transition-all cursor-pointer shadow-md"
                                    >
                                        Ver no mapa
                                    </button>
                                )}
