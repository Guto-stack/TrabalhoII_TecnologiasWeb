...
export function Estatisticas() {
  ...
  useEffect(() => {
    fetch("http://localhost:3333/estatisticas")
      ...
      .then((json) => {
        ...
        if (json.length > 0) {
          setActivePeriodId(json[0].id_periodo); // Inicializa com o primeiro período
        }
        setLoading(false);
      })
      .catch((err) => {
  ...
  if (loading) {
    return (
      <div className="w-full flex flex-col items-center justify-center p-12 text-text-main/70">
  ...
  if (periodosDados.length === 0) {
    return (
      <div className="p-8 text-center text-text-main/60 flex flex-col items-center gap-2">
  ...
  // Localiza no estado os dados correspondentes ao período que está clicado
  ...
  return (
    <div className="w-full flex flex-col md:flex-row gap-6 items-start">
      ...
      <div className="flex flex-col gap-3 w-full md:w-64 shrink-0">
        <h2 className="text-text-main font-bold text-xs tracking-wider uppercase mb-1 opacity-80 pl-1">
          Selecionar Era
        </h2>
        {periodosDados.map((p) => (
          <button
      ...
      <div className="flex-1 w-full flex flex-col gap-6 animate-fadeIn">
        ...
        {/*Dados das especies pelo periodo selecionado */}
                ...
                <tbody>
                  {periodoSelecionado.tabelaClados.map((c, i) => (
                    <tr key={i} className="border-b border-accent/5 hover:bg-header/30 transition-colors">
                      ...
                      <td className="py-3 px-2 text-right font-medium text-amber-500/90">{c.pesoMedio > 0 ? `${c.pesoMedio.toLocaleString()} kg` : "—"}</td>
              ...
              {periodoSelecionado.topGigantes.map((d, index) => (
                <div key={d.id} className="relative bg-header/40 border border-accent/10 p-4 rounded-xl flex flex-col justify-between">
                  ...
                  <div className="mt-4 pt-3 border-t border-accent/5 flex flex-col gap-1 text-xxs text-text-main/60">
                    ...
                    <div className="flex justify-between"><span>Peso Estimado:</span><span className="font-bold text-amber-500">{d.peso ? `${d.peso.toLocaleString()} kg` : "—"}</span></div>
