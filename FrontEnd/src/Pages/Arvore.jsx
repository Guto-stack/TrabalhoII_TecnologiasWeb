...
export function Arvore({ isDarkMode, alternarTema }) {
  ...
  const handleNodeClick = (event, clickedNode) => {
    if(clickedNode.data.isExpanded){
      ...
      const nextNodes = nodes
        .filter(node => !descendantsToRemove.includes(String(node.id)))
        .map(node => String(node.id) === String(clickedNode.id) ? { ...node, data: { ...node.data, isExpanded: false } } : node);
      
      const nextEdges = edges.filter(edge => !descendantsToRemove.includes(String(edge.target)));
      ...
      const conexoesFilhas = bancoLocal.edges.filter(e => String(e.source) === String(clickedNode.id));

      if(conexoesFilhas.length === 0) return; 

      const listaFilhosIds = conexoesFilhas.map(e => String(e.target));
      const filhosParaRenderizar = bancoLocal.nodes.filter(n => listaFilhosIds.includes(String(n.id)));

      const novosFilhosFiltrados = filhosParaRenderizar.filter(child => !nodes.some(n => String(n.id) === String(child.id)));
      ...
      if(novosFilhosFiltrados.length > 0){
        ...
        const nextNodes = [
          ...nodes.map(node => String(node.id) === String(clickedNode.id) ? { ...node, data: { ...node.data, isExpanded: true } } : node),
          ...newNodes
        ];
